...
GREEN='\033[0;32m'
NC='\033[0m' # No Color
export WLF_DIRECTORY=/tmp/HOTROD_JDG
...
# Profile
# ========================
if [[ "x$1" = "x--coarse" ]]; then
    export WLF_CLI_SCRIPT=configuration-wfl-SESSION.cli
    export MVN_PROFILE="-q -P coarse"
...
elif [[ "x$1" = "x--fine" ]]; then
    export WLF_CLI_SCRIPT=configuration-wfl-ATTRIBUTE.cli
    export MVN_PROFILE="-q -P fine"
    export WAR_FINAL_NAME=distributed-webapp.war
    export WAR_CONTEXT_PATH=distributed-webapp
    export JDG_CLI_SCRIPT=configuration-jdg.cli
else
